...
/* raycast minimum intensity projection */
...
void main(void)
{
  ...
  do
  {
    ...
    min_val = min(min_val, texture(volume_texunit, ray_pos).r);
  }
  while(true);

  if (min_val <= val_threshold)
  {
    frag_output = texture(trfunc_texunit, min_val);
  }
  else
  {
    discard;
